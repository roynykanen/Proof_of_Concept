...
export default function Home() {
  ...
  // Check once on mount whether SLACK_WEBHOOK_URL is set server-side
  useState(() => {
    fetch("/api/slack").then((r) => r.json()).then(({ configured }) => setSlackConfigured(configured));
  });
          ...
          <>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4" data-no-print>
              <div className="flex items-center gap-2.5 shrink-0">
                ...
                {slackConfigured && (
                  <span className="text-xs text-emerald-600 font-medium bg-emerald-50 px-2 py-0.5 rounded-full">configured</span>
                )}
              </div>
              ...
              {slackConfigured && slackStatus === "idle" && (
                <p className="flex-1 text-sm text-gray-400">Webhook set via environment variable.</p>
              )}

              <button
                onClick={sendToSlack}
                disabled={(!slackConfigured && !slackUrl) || slackStatus === "sending" || slackStatus === "sent"}
